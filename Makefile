...
DOCKERFILE_PROD  = hack/docker/mc-operator/in.Dockerfile
DOCKERFILE_DBG   = hack/docker/mc-operator/dbg.Dockerfile
...
install:
	@cd ../installer; \
	APPSCODE_ENV=dev KUBEDB_OPERATOR_TAG=$(TAG) KUBEDB_CATALOG=memcached ./deploy/kubedb.sh --operator-name=$(BIN) --docker-registry=$(REGISTRY) --image-pull-secret=$(REGISTRY_SECRET)
